function trialseq = ALTH_backend(settings,trialseq,data)
...
glo_welcome(ow);
    ...
    for it = 1:size(trialseq,1)
        ...
        if trialseq(it,id.nov) == 1
            [rt,resp] = glo_getmouse(trialseq(it,id.deadline), settings.fh, settings.duration.fhoffset, settings.fh1, settings.duration.fh1offset, settings);
        else
            [rt,resp] = glo_getmouse(trialseq(it,id.deadline));
        end

        % Just in case response happens between functions, handles won't turn
        % off, this is a safety net.
        ...
        if trialseq(it,id.go) == 1 && resp == 0
            ...
            Screen('Flip', settings.screen.outwindow);
            [~, post_resp] = glo_getmouse(settings.duration.post_check);
        end
        
        % Code Responses
        if trialseq(it,id.go) == 1
            if resp ~= 0 && resp == trialseq(it,id.side)
                trialseq(it,id.acc) = 1; % correct
            elseif resp ~= 0 && resp ~= trialseq(it,id.side)
                trialseq(it,id.acc) = 2; % error
            elseif resp == 0
                trialseq(it,id.acc) = 99; % miss
            end
        else
            if resp == 0 && post_resp == 0
        ...
        % Record RT/resp
        trialseq(it,id.rt) = rt*1000;
        trialseq(it,id.resp) = resp;
