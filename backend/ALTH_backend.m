...
glo_countdown(ow);
    ...
    for it = 1:size(trialseq,1)
        ...
        % Check for Response
        [trialseq(it,id.rt),trialseq(it,id.resp)] = handle_response_ALTH(settings.daq,trialseq(it,id.deadline)*1000,settings,trialseq,id,it);
        ...
        if trialseq(it,id.go) == 1 && trialseq(it,id.resp) == 0
            ...
            Screen('Flip', settings.screen.outwindow);
            [~,post_resp] = handle_response_ALTH_postcheck(settings.daq,500);
        end
